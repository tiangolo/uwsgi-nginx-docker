...
**Docker** image with **uWSGI** and **Nginx** for **Python 2.7** and **Python 3.6** applications (as **Flask**) in a single container.
...
* 2016-08-16: There's now an image tag for Python 3.6, based on the official image for Python 3.6. So now you can use this image for your projects in Python 2.7 and Python 3.6.
...
* Alternatively, you could use `FROM tiangolo/uwsgi-nginx:python2.7`, but this is the default (it is also marked with the `latest` tag), so the result is the same as the method above. Python 2.7 is the default as it is still the most used version of Python and [not necessarily all the packages are available for Python 3.6](http://flask.pocoo.org/docs/0.11/python3/).

If you want to use it as a base image for a Python 3.6 application then you should use `FROM tiangolo/uwsgi-nginx:python3.6` in your `Dockerfile`.
